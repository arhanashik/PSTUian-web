...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="mb-4">
        <button type="button" class="btn btn-primary float-end" data-bs-toggle="modal" data-bs-target="#data-add-modal">
        Add New Batch
        </button>
...
<script>
    ...
    function generateTr(faculty) {
        ...
        var btnEdit = `<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#data-edit-modal" data-id="${faculty.id}" data-short-title="${faculty.short_title}" data-title="${faculty.title}">Edit</button>`;
        var btnRestore = `<button class="btn btn-success" onclick='restoreFaculty(` + param + `)'>Restore</button>`;
        var btnDelete = `<button class="btn btn-danger" onclick='deleteFaculty(` + param + `)'>Delete</button>`;
        return `<tr id="${faculty.id}">` + 
        `<th scope="row">${faculty.id}</th>` +
        `<td>${faculty.short_title}</td>` +
        `<td>${faculty.title}</td>` +
        `<td id="td-deleted-${faculty.id}">${deleted}</td>` +
