...
class CheckInDb extends Db
{
    ...
    public function getAll($page, $limit)
    {
        ...
        $columns = "ci.id, ci.user_id, ci.user_type, ci.count, ci.updated_at, 
        s.name, s.phone, s.image_url, b.name as batch";
        $sql = "SELECT $columns FROM " . CHECK_IN_TABLE . " ci ";
        // join
        $sql .= " LEFT JOIN " . STUDENT_TABLE . " s ON s.id = ci.user_id";
        $sql .= " LEFT JOIN " . BATCH_TABLE . " b ON b.id = s.batch_id";
        // conditions
        $sql .= " WHERE ci.visible = 1 AND ci.deleted = 0";
        // sorting
    ...
    public function getById($id)
    {
        ...
        $sql = "SELECT $columns FROM " . CHECK_IN_TABLE . " bdr ";
        // join
    ...
    {
        $sql = "INSERT INTO " . CHECK_IN_TABLE . "(user_id, user_type, blood_group, before_date, contact, info) 
    ...
    {
        $sql = "UPDATE " . CHECK_IN_TABLE . " SET blood_group = '$blood_group', 
